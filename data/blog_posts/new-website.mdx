...
<Popout>
  9/18 Edit: Now I know why that ☝️ didn't work! React server components render
  once, hydrated and all (as opposed to SRR, which just renders an unhydrated
  'shell'). There's no way for a server component to run the JS client-side to
  do any kind of "fetch this every 5 seconds" thing. Thanks to Josh Comeau's
  awesome [article](https://joshwcomeau.com/react/server-components/) about this
  stuff that cleared it up for me!
</Popout>
